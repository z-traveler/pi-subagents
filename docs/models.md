...
The interactive main agent is advisory-only: pi-subagents never aborts or changes its model. When a response is slow, pi-subagents opens a fixed-width overlay in the lower right, above the editor, once per model per session. It closes automatically after `mainAdvisoryDurationMs` (30 seconds by default), or immediately with Esc. Set that value to `0` to suppress the overlay while keeping performance monitoring and background probes. If the current provider/model maps (ignoring thinking level) to exactly one configured class, the overlay recommends another candidate from that class, shows its fresh cached throughput (or `no fresh sample`), and may probe alternatives for future routing; otherwise it shows a generic warning. The advisory is not inserted into model context or session entries.

Use `/subagents-model-performance` in the Pi TUI to inspect every configured model class's cached first-token and generation-throughput measurements and candidate order in a compact fixed-width centered overlay. Fresh measurements determine the order; candidates without fresh samples retain their order from settings. Throughput is colored against the configured hard and soft thresholds. The cache expires after `cacheTtlMs` (five minutes by default), so a previously measured candidate can show `no fresh sample`. The command reads the cache without starting a new probe or switching the main model. Press Esc to close the overlay.
